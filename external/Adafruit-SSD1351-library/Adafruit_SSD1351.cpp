...
Adafruit_SSD1351::Adafruit_SSD1351(uint16_t width, uint16_t height, nrf_drv_spi_t* pSpi,
...
/*!
    ...
    @return  None (void).
*/
void Adafruit_SSD1351::begin(nrf_drv_spi_frequency_t frequency /*  = NRF_DRV_SPI_FREQ_8M */) {

  initSPI(frequency);

  const uint8_t *pAddr = initList;
  uint8_t        cmd, x, numArgs;

  while((cmd = *pAddr++) > 0) { // '0' command ends list
    x = *pAddr++;
    numArgs = x & 0x7F;
    if (cmd != 0xFF) { // '255' is ignored
      sendCommand(cmd, pAddr, numArgs);
    }
    pAddr += numArgs;
  }
  setRotation(0);
...
*/
void Adafruit_SSD1351::invert(bool i) {
  invertDisplay(i);
}
