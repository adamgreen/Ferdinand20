...
// Singleton used to determine which object _spiHandler should use when SPI events come in.
Adafruit_SPITFT* Adafruit_SPITFT::g_pSingleton = NULL;

// CONSTRUCTORS ------------------------------------------------------------
/*!
    @brief   Adafruit_SPITFT constructor for hardware SPI using a specific
             SPI peripheral.
    @param   width     Display width in pixels at default rotation (0).
    @param   height    Display height in pixels at default rotation (0).
    @param   pSpi      Pointer to nRF SPI object.
    @param   mosiPin   Pin # for MOSI.
    @param   sckPin    Pin # for SCLK.
    @param   cs        Pin # for chip-select (-1 if unused, tie CS low).
    @param   dc        Pin # for data/command select (required).
    @param   rst       Pin # for display reset (optional, display reset
    ...
    @note    Output pins are not initialized in constructor; application
             ...
             pins.
*/
Adafruit_SPITFT::Adafruit_SPITFT(uint16_t width, uint16_t height, nrf_drv_spi_t* pSpi,
                                 uint8_t mosiPin, uint8_t sckPin, uint8_t csPin, uint8_t dcPin,
                                 uint8_t rstPin /* = NRF_DRV_SPI_PIN_NOT_USED */)
    : Adafruit_GFX(width, height), m_pSpi(pSpi), m_mosiPin(mosiPin), m_sckPin(sckPin), m_rstPin(rstPin),
      m_csPin(csPin), m_dcPin(dcPin), m_startCount(0), m_finCount(0)
{
    g_pSingleton = this;
}
// end constructors -------
...
/*!
    ...
    @param  frequency SPI frequency when using hardware SPI.
    @param  mode     SPI mode when using hardware SPI.
*/
void Adafruit_SPITFT::initSPI(nrf_drv_spi_frequency_t frequency /* = NRF_DRV_SPI_FREQ_8M */,
                              nrf_drv_spi_mode_t mode /* = NRF_DRV_SPI_MODE_0 */)
{
    nrf_drv_spi_config_t spiConfig = NRF_DRV_SPI_DEFAULT_CONFIG;
    spiConfig.mosi_pin = m_mosiPin;
    spiConfig.sck_pin  = m_sckPin;
    spiConfig.frequency = frequency;
    spiConfig.mode = mode;
    APP_ERROR_CHECK(nrf_drv_spi_init(m_pSpi, &spiConfig, _spiHandler));
    ...
    if (m_rstPin != NRF_DRV_SPI_PIN_NOT_USED)
    {
        // Toggle reset low to reset
        nrf_gpio_cfg_output(m_rstPin);
        nrf_gpio_pin_set(m_rstPin);
        nrf_delay_ms(100);
        nrf_gpio_pin_clear(m_rstPin);
        nrf_delay_ms(100);
        nrf_gpio_pin_set(m_rstPin);
        nrf_delay_ms(200);
    }
...
*/
void Adafruit_SPITFT::writePixels(const uint16_t *colors, uint32_t len, bool block, bool bigEndian)
{
    while (len--) {
        spiWrite16(*colors++);
    }
...
{
    m_buffer.writeData(b);
    startTransmission();
}

void Adafruit_SPITFT::spiWrite16(uint16_t w)
{
...
void Adafruit_SPITFT::writeCommand(uint8_t cmd) {
    m_buffer.writeCommand(cmd);
    startTransmission();
}


void Adafruit_SPITFT::spiHandler(nrf_drv_spi_evt_t const * pEvent)
{
    if (pEvent) {
        m_finCount++;
    }
    while (1) {
        ...
        bool result = m_buffer.readBytes(&pBytes, &len, &flags);
        if (!result) {
            return;
        }
        if (m_buffer.isEnd(flags)) {
            nrf_gpio_pin_set(m_csPin);
            continue;
        }
        if (m_buffer.isStart(flags)) {
            nrf_gpio_pin_clear(m_csPin);
        }
        if (m_buffer.isCommand(flags)) {
        ...
        m_startCount++;
        APP_ERROR_CHECK(nrf_drv_spi_transfer(m_pSpi, pBytes, len, NULL, 0));
        return;
    }
}
